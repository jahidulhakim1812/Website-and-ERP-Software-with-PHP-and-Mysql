...
/**
 * mirage_report.php
 * MIRAGE LOD‑3 – Report: search by subcontractor and view detailed batch information.
 * Grand total = total due across all batches (costs - payments).
 * Also shows total pieces and total additional costs.
 */
...
if ($selected_sub_id > 0) {
    ...
    // For each batch, get items, costs, and payments
    foreach ($batches as &$batch) {
        ...
        $batch['due'] = $batch['costs_subtotal'] - $batch['paid'];
        if ($batch['due'] < 0) $batch['due'] = 0;
        $batch['payment_status'] = ($batch['due'] == 0) ? 'Fully Paid' : 'Partial Paid';
    }
    unset($batch);

    // Calculate grand totals
    $grand_due   = array_sum(array_column($batches, 'due'));
    $grand_costs = array_sum(array_column($batches, 'costs_subtotal'));
...
<head>
    ...
    <title>MIRAGE Report | NexusAdmin</title>
    ...
    <main class="main-content">
        <header class="top-header">
            <div style="display: flex; align-items: center; gap: 16px;">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span style="font-size: 0.9rem; color: var(--text-muted);">MIRAGE Report</span>
                </div>
            </div>
        ...
        <div class="scrollable">
            <div class="sheet-container">
                <h1><i class="ph ph-magnifying-glass"></i> MIRAGE Production Report</h1>
                <div class="subtitle">View batches by subcontractor – detailed breakdown of sizes, quantities, and costs. <strong>Grand total shows total due</strong> (additional costs minus payments).</div>
                ...
                <div class="search-bar">
                    ...
                    <form method="get" style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        <?php if ($selected_sub_id): ?>
                            <a href="mirage_report.php" class="btn btn-secondary"><i class="ph ph-x-circle"></i> Clear</a>
                        <?php endif; ?>
                        <!-- Print button linking to print‑optimized page -->
                        <?php if ($selected_sub_id): ?>
                            <a href="mirage_report_print.php?subcontractor_id=<?= $selected_sub_id ?>" target="_blank" class="btn btn-secondary"><i class="ph ph-printer"></i> Print Report</a>
                        <?php endif; ?>
                    </form>
                </div>

                <?php if ($selected_sub_id && $selected_sub_name): ?>
                    <h2 style="margin-bottom: 15px;">Batches for: <?= e($selected_sub_name) ?></h2>

                    <?php if (empty($batches)): ?>
                        <div class="no-data">No batches found for this subcontractor.</div>
                    <?php else: ?>
                        <?php foreach ($batches as $batch): ?>
                            <div class="batch-card">
                                ...
                                <?php if (!empty($batch['items'])): ?>
                                    <h4 style="margin-bottom: 10px;">Wig Pieces</h4>
                                    <table>
                                        <thead>
                                            <tr>
                                                <th>Type</th>
                                ...
                                <div class="payment-info">
                                    <div>
                                        <span>Total Cost: <?= number_format($batch['costs_subtotal'], 2) ?> BDT</span> |
                                        <span>Paid: <?= number_format($batch['paid'], 2) ?> BDT</span> |
                                        <span>Due: <?= number_format($batch['due'], 2) ?> BDT</span>
                                    </div>
                                    <span class="status-badge <?= $batch['due'] == 0 ? 'status-fully-paid' : 'status-partial' ?>">
                        ...
                        <div class="grand-total-box">
                            ...
                            <span><?= number_format($grand_due, 2) ?> BDT</span>
                        </div>

                        <!-- Additional summary (costs & pieces) -->
                        <div class="cost-summary">
                            <span>Total Additional Costs: <?= number_format($grand_costs, 2) ?> BDT</span>
                            <span>Total Pieces: <?= number_format($grand_pieces) ?></span>
                        </div>
                    <?php endif; ?>

                <?php elseif ($selected_sub_id && !$selected_sub_name): ?>
                    <div class="no-data">Subcontractor not found.</div>
                <?php else: ?>
                    <div class="no-data">Please select a subcontractor to view the report.</div>
                <?php endif; ?>
            </div>
        </div>
    </main>
