<?php
/**
 * add_subcontractor.php
 * Add New Subcontractor Page with Photo & NID – Wig Store Specializations
 */
...
// Create subcontractors table with new columns if not exists
if (tableExists($pdo, 'subcontractors') === false) {
    $sql = "CREATE TABLE IF NOT EXISTS subcontractors (
    ...
    // If table exists but columns might be missing, add them (optional safety)
    try {
        $pdo->query("SELECT nid_number FROM subcontractors LIMIT 1");
    } catch (Exception $e) {
        $pdo->exec("ALTER TABLE subcontractors ADD COLUMN nid_number VARCHAR(100) AFTER tax_id");
    }
    try {
        $pdo->query("SELECT photo FROM subcontractors LIMIT 1");
    } catch (Exception $e) {
        $pdo->exec("ALTER TABLE subcontractors ADD COLUMN photo VARCHAR(255) AFTER nid_number");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $pdo->prepare("SELECT id FROM subcontractors WHERE email = ?");
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            $photo_path = 'uploads/subcontractors/' . $new_filename; // relative path for DB
        ...
        $sql = "INSERT INTO subcontractors (
            company_name, contact_person, email, phone, address, specialization,
            tax_id, nid_number, photo, registration_date, project_rate,
            status, notes
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $subcontractor_id = $pdo->lastInsertId();
        
        $message = "Subcontractor added successfully! ID: #" . $subcontractor_id;
        $msg_type = 'success';
        
        // Clear form data on success (but keep if "Add Another" is used)
        if (!isset($_POST['add_another'])) {
            $form_data = [];
        } else {
            $form_data = $_POST; // keep for next entry
            // but don't keep file path
        }
...
<head>
    ...
    <title>Add Subcontractor | Wig Store</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <header class="top-header">
            <div style="display: flex; align-items: center; gap: 16px;">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <span style="font-size: 0.9rem; color: var(--text-muted);">Add New Subcontractor</span>
                </div>
            </div>
        ...
        <div class="scrollable">
            <div class="page-header">
                <div>
                    <h1 class="page-title">Add New Subcontractor</h1>
                    <p class="page-subtitle">
                        Register a new subcontractor to your wig store. Fill in the required information below.
                    </p>
                </div>
                <div>
            ...
            <?php
            try {
                $total_subs = $pdo->query("SELECT COUNT(*) as count FROM subcontractors")->fetch()['count'];
                $active_subs = $pdo->query("SELECT COUNT(*) as count FROM subcontractors WHERE status = 'Active'")->fetch()['count'];
                $pending_subs = $pdo->query("SELECT COUNT(*) as count FROM subcontractors WHERE status = 'Pending'")->fetch()['count'];
                $inactive_subs = $pdo->query("SELECT COUNT(*) as count FROM subcontractors WHERE status = 'Inactive'")->fetch()['count'];
            } catch (Exception $e) {
                $total_subs = $active_subs = $pending_subs = $inactive_subs = 0;
            }
            ?>
            <div class="quick-stats">
                <div class="stat-box">
                    <div class="stat-number"><?php echo $total_subs; ?></div>
                ...
                <div class="stat-box">
                    <div class="stat-number"><?php echo $pending_subs; ?></div>
                    <div class="stat-label">Pending</div>
                </div>
                <div class="stat-box">
                    <div class="stat-number"><?php echo $inactive_subs; ?></div>
            ...
            <div class="form-container">
                ...
                <form method="POST" action="" enctype="multipart/form-data" class="form-body">
                    <div class="form-grid">
                        ...
                        <!-- WIG STORE SPECIALIZATION -->
                        <div class="form-group">
                            <label>Specialization</label>
                            <select name="specialization">
                                <option value="">Select Specialization</option>
                                <option value="Lace Front Wigs" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Lace Front Wigs' ? 'selected' : ''; ?>>Lace Front Wigs</option>
                                <option value="Full Lace Wigs" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Full Lace Wigs' ? 'selected' : ''; ?>>Full Lace Wigs</option>
                                <option value="Synthetic Wigs" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Synthetic Wigs' ? 'selected' : ''; ?>>Synthetic Wigs</option>
                                <option value="Human Hair Wigs" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Human Hair Wigs' ? 'selected' : ''; ?>>Human Hair Wigs</option>
                                <option value="Wig Customization" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Wig Customization' ? 'selected' : ''; ?>>Wig Customization</option>
                                <option value="Wig Styling" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Wig Styling' ? 'selected' : ''; ?>>Wig Styling</option>
                                <option value="Wig Repair" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Wig Repair' ? 'selected' : ''; ?>>Wig Repair</option>
                                <option value="Wig Cleaning" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Wig Cleaning' ? 'selected' : ''; ?>>Wig Cleaning</option>
                                <option value="Wig Manufacturing" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Wig Manufacturing' ? 'selected' : ''; ?>>Wig Manufacturing</option>
                                <option value="Other" <?php echo isset($form_data['specialization']) && $form_data['specialization'] == 'Other' ? 'selected' : ''; ?>>Other</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="registration_date" 
                                   value="<?php echo e($form_data['registration_date'] ?? $today); ?>"
                                   id="regDate">
                        </div>
            ...
            <div class="form-tips">
                ...
                <ul>
                    ...
                    <li>Specializations are tailored for wig store vendors and service providers</li>
                    ...
                    <li>You can upload a photo (JPEG, PNG, GIF, WEBP) up to 2MB</li>
                    <li>NID card number is optional but recommended for identification</li>
                    <li>Use "Save & Add Another" to quickly add multiple subcontractors</li>
                </ul>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        // Auto-fill Specialization "Other"
        const specializationSelect = document.querySelector('select[name="specialization"]');
        const otherSpecialization = document.createElement('input');
        otherSpecialization.type = 'text';
        otherSpecialization.name = 'specialization_other';
        otherSpecialization.placeholder = 'Specify specialization';
        otherSpecialization.style.marginTop = '8px';
        otherSpecialization.style.display = 'none';
        
        if (specializationSelect) {
            specializationSelect.parentNode.appendChild(otherSpecialization);
            
            specializationSelect.addEventListener('change', function() {
                if (this.value === 'Other') {
                    otherSpecialization.style.display = 'block';
                    otherSpecialization.required = true;
                } else {
                    otherSpecialization.style.display = 'none';
                    otherSpecialization.required = false;
                }
            });
