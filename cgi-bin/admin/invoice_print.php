...
$IMG_BASE = ''; // prefix to product_img if stored relative, e.g. '/admin/'
$COMPANY = [
    'name' => 'Ali Hair Wigs',
...
// DEBUG: set true to show DB errors in JSON responses (only for dev)
...
if (!empty($orderItems)) {
    foreach ($orderItems as $it) {
        ...
        $img = $has_img ? ($it['product_img'] ?? '') : ($order['product_img'] ?? '');
        $meta = $has_meta ? ($it['meta'] ?? null) : null;
    ...
    if (!empty($order['items']) && is_string($order['items'])) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
            foreach ($decoded as $it) {
                ...
                $items[] = ['name'=>$name,'qty'=>$qty,'price'=>$unit,'line'=>$line,'img'=>$it['img'] ?? '','meta'=>null];
            }
        }
    }
    if (empty($items)) {
    ...
    <div class="header">
      <div class="brand">
        ...
        <div>
          ...
          <div class="meta">Phone: <?php echo esc($COMPANY['phone']); ?> · Email: <?php echo esc($COMPANY['email']); ?></div>
        </div>
      </div>
        ...
        <tr>
          <td>
            <div class="items-desc">
              <?php if ($img !== ''): ?>
                <img src="<?php echo esc(rtrim($IMG_BASE, '/') . '/' . ltrim($img, '/')); ?>" alt="" class="img-thumb" onerror="this.style.display='none'">
              <?php endif; ?>
              <div>
...
(function(){
  ...
  document.getElementById('doPrint').addEventListener('click', triggerPrint);
})();
</script>
</body>
</html>
