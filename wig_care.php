...
<head>
  ...
  <title>Wig Care — Ali Hair Wigs</title>
...
<main class="page-inner" role="main">
  <div class="hero">
    <h1>Wig Care</h1>
    <p class="lead">Essential care tips to keep your wig natural-looking, comfortable and long-lasting.</p>
  </div>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">Everyday Care</h2>
    <p class="muted">Brush gently with a wide-tooth comb; avoid sleeping in your wig; protect from strong perfumes and high heat.</p>
  </section>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">Washing & Conditioning</h2>
    <p class="muted">Use cool water and wig-specific shampoo. For human-hair wigs, condition mid-length to ends and rinse thoroughly. Pat dry with a towel and air-dry on a stand.</p>
  </section>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">Styling Advice</h2>
    <p class="muted">Use low heat and a heat protectant on human-hair wigs. For synthetic wigs, use only products labeled heat-friendly. Avoid heavy styling products that cause build-up.</p>
  </section>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">Lace Front & Adhesive Care</h2>
    <p class="muted">Remove adhesive gently with approved removers. Clean lace carefully and avoid overstretching. Replace adhesives as recommended.</p>
  </section>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">Storage & Travel</h2>
    <p class="muted">Store on a wig stand or in a breathable bag away from sunlight and humidity. For travel, place in a protective box or soft bag to preserve shape.</p>
  </section>

  <section class="section">
    <h2 style="font-weight:800;margin-bottom:8px">When to Seek Professional Help</h2>
    <p class="muted">For color changes, major restyling, or repairs (re-knotting, lace repair) consult a professional wig technician to avoid irreversible damage.</p>
  </section>

  <section class="section" style="display:flex;gap:16px;flex-wrap:wrap;align-items:center">
    <div style="flex:1;min-width:240px">
      <h3 style="margin:0 0 8px;font-weight:800">Need personalised guidance?</h3>
      <p class="muted">Share your wig details and we'll recommend the best care routine.</p>
    </div>
...
<aside id="cart-drawer" class="cart-drawer" aria-hidden="true" role="dialog" aria-label="Shopping cart">
  ...
  <div class="cart-footer cart-footer-sticky p-4 border-t">
...
<script>
/* Functional search + cart logic (localStorage). No external tools required. */
(function(){
  // Mobile drawer
  ...
  function openMobile(){ if(!mobileDrawer) return; mobileDrawer.classList.add('open'); mobileBackdrop.classList.add('visible'); mobileDrawer.setAttribute('aria-hidden','false'); if(openIcon) openIcon.style.display='none'; if(closeIcon) closeIcon.style.display='block'; document.documentElement.style.overflow='hidden'; document.body.style.overflow='hidden'; }
  function closeMobile(){ if(!mobileDrawer) return; mobileDrawer.classList.remove('open'); mobileBackdrop.classList.remove('visible'); mobileDrawer.setAttribute('aria-hidden','true'); if(openIcon) openIcon.style.display='block'; if(closeIcon) closeIcon.style.display='none'; document.documentElement.style.overflow=''; document.body.style.overflow=''; hamburger.focus(); }
  ...
  if (searchForm) {
    // Validate and submit; keep default navigation to search.php
    searchForm.addEventListener('submit', function(e){
      const q = (searchInput && searchInput.value || '').trim();
      if (!q) { e.preventDefault(); searchInput.focus(); return; }
      closeSearch();
      // form will submit naturally and navigate to search.php?q=...
    });
  }
  ...
  // Global delegated handler for add-to-cart buttons (works anywhere on page)
  ...
  // initial render and keep UI in sync if other tabs change storage
  renderCart();
  window.addEventListener('storage', ()=> { cart = loadCart(); renderCart(); });

  // expose close functions for outside usage
