...
/*
 women_wigs.php
 - Shows ONLY Women products
 - Uses same header, footer, cart, search and mobile drawer UI/JS as men_wigs.php
 - Side panel contains Women-related categories (featured box removed)
 - Schema-robust: supports p.category (string), p.category_id (numeric), or join table patterns
 - Pagination, sorting, mobile drawer, search overlay, slide-out cart
 - Configure DB constants below
*/
...
/* If you know the product_category id that represents "Women", set it here. Leave 0 to auto-detect. */
define('FORCE_WOMEN_CATEGORY_ID', 0);
...
/* -------------------------------
   Resolve Women category id (optional)
   ------------------------------- */
$womenCategoryId = (int)FORCE_WOMEN_CATEGORY_ID;
if ($womenCategoryId <= 0 && !empty($categories)) {
    foreach ($categories as $c) {
        $slug = isset($c['slug']) ? mb_strtolower((string)$c['slug'],'UTF-8') : null;
        $name = isset($c['name']) ? mb_strtolower((string)$c['name'],'UTF-8') : null;
        if ($slug === 'women' || (strpos((string)$name,'women') !== false)) {
            $womenCategoryId = (int)$c['id'];
            break;
        }
    }
}
...
/* -------------------------------
   Build WHERE for Women products (or apply side-panel selected category if provided)
   ------------------------------- */
$where = [];
$params = [];
$types = '';
...
if ($filterCategoryId > 0) {
    // user clicked a category in side panel (which will be limited to Women-related categories)
    $selCat = null;
    ...
    // ALSO ensure results remain Women-only (defensive): require women-match as well when possible
    $womenSub = [];
    if ($has('category')) $womenSub[] = "LOWER(p.category) = 'women'";
    if ($has('tags')) { $womenSub[] = 'LOWER(p.tags) LIKE ?'; $types .= 's'; $params[] = '%women%'; }
    if ($has('name')) { $womenSub[] = 'LOWER(p.name) LIKE ?'; $types .= 's'; $params[] = '%women%'; }
    if ($womenSub) $where[] = '(' . implode(' OR ', $womenSub) . ')';

} else {
    // No specific category selected: show Women products
    if ($has('category')) {
        $where[] = "LOWER(p.category) = 'women'";
    } elseif ($has('category_id')) {
        if ($womenCategoryId <= 0) {
            // fallback to tags/name containing "women"
            $sub = [];
            if ($has('tags')) { $sub[] = 'LOWER(p.tags) LIKE ?'; $types .= 's'; $params[] = '%women%'; }
            if ($has('name')) { $sub[] = 'LOWER(p.name) LIKE ?'; $types .= 's'; $params[] = '%women%'; }
            if ($sub) $where[] = '(' . implode(' OR ', $sub) . ')';
            else {
                echo "<!doctype html><html><body><h1>Women category id not found. Set FORCE_WOMEN_CATEGORY_ID or ensure product_category contains a Women entry.</h1></body></html>";
                exit;
            }
        } else {
            $where[] = 'p.category_id = ?';
            $types .= 'i';
            $params[] = $womenCategoryId;
        }
    } elseif ($haveJoin) {
        if ($womenCategoryId <= 0) {
            echo "<!doctype html><html><body><h1>Women category id not found for join table match. Set FORCE_WOMEN_CATEGORY_ID or ensure product_category contains a Women entry.</h1></body></html>";
            exit;
        }
        $jt = $joinTable;
        ...
        if ($sub) $where[] = '(' . implode(' OR ', $sub) . ')';
            ...
            echo "<!doctype html><html><body><h1>Cannot reliably filter products to Women because products table lacks category/category_id/tags/name and no join table found.</h1></body></html>";
            exit;
        }
    }
}
...
/* -------------------------------
   ...
   We'll use these counts to determine which categories relate to Women products.
   ------------------------------- */
$catCounts = [];
if (!empty($categories)) {
...
/* -------------------------------
   Filter categories to those related to Women products
   Criteria:
   - category name or slug contains 'women' OR
   - category id equals resolved Women category id OR
   - category has non-zero product count in $catCounts
   Result: $categories will only contain Women-related categories for side panel
   ------------------------------- */
$womenCategories = [];
if (!empty($categories)) {
    foreach ($categories as $c) {
        ...
        $isWomenName = (strpos($name, 'women') !== false) || ($slug === 'women') || (strpos($slug, 'women') !== false);
        $hasCount = isset($catCounts[$cid]) && $catCounts[$cid] > 0;
        if ($isWomenName || $cid === $womenCategoryId || $hasCount) {
            $womenCategories[] = $c;
        }
    }
    $categories = array_values(array_unique($womenCategories, SORT_REGULAR));
}

/* -------------------------------
   Render HTML (header/footer and UI match men_wigs.php)
   Note: featured box removed from side panel
   ------------------------------- */
?><!doctype html>
<html lang="en">
<head>
  ...
  <title>Women's Wigs — Ali Hair Wigs</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    @media(min-width:1024px){ .page-grid{grid-template-columns:300px 1fr} }

    .side-panel{background:#fff;border:1px solid #eef2f6;border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px}
    .side-title{font-weight:800;color:var(--primary);margin-bottom:6px}
    ...
    /* Fullscreen category-loading overlay for mobile category taps */
    ...
    #mobileCategoryOverlay.open { display: flex; }
    #mobileCategoryOverlay .panel {
      background: #fff;
      color:#111827;
      border-radius: 12px;
      padding: 18px 22px;
    ...
    #mobileCategoryOverlay .spinner {
      width:36px; height:36px; border-radius:50%; border:4px solid #f3f3f3; border-top-color: var(--primary); animation:spin .8s linear infinite;
    }
    @keyframes spin { to { transform: rotate(360deg); } }

    /* Cart footer buttons styling (updated to match screenshot) */
    .cart-footer { padding: 14px; border-top: 1px solid #eef2f6; background: #fff; }
    .checkout-btn {
      ...
      box-shadow: 0 6px 18px rgba(123,63,0,0.12);
      margin-bottom:10px;
    }
    .continue-btn {
      display:inline-block;
    ...
    @media(min-width:420px){
      .checkout-btn, .continue-btn { width:auto; min-width:160px; }
      .cart-footer .actions { display:flex; gap:8px; justify-content:flex-end; align-items:center; }
    }
  </style>
</head>
...
<header class="site-header" role="banner">
  <div class="header-inner">
    ...
    <button id="hamburger" aria-expanded="false" aria-controls="mobile-left-drawer" class="hamburger md:hidden p-2 rounded-md text-gray-700 hover:bg-gray-100" type="button">
...
<!-- Mobile category-opening overlay -->
<div id="mobileCategoryOverlay" aria-hidden="true">
  <div class="panel" role="status" aria-live="polite">
    <div class="spinner" aria-hidden="true"></div>
    <div id="mobileCategoryOverlayText">Opening category...</div>
  </div>
</div>
  ...
  <div class="page-grid" role="region" aria-label="Products and categories">
    ...
    <aside class="side-panel" aria-label="Product categories">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        ...
        <!-- Mobile-only toggle (visible <1024px) -->
        <button id="cat-toggle" aria-expanded="false" class="cat-toggle-btn" type="button">
          Categories
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" style="margin-left:6px">
      ...
      <!-- Featured box intentionally removed as requested -->
    ...
    <section>
      <div class="grid" id="productGrid">
        <?php if (empty($products)): ?>
          <div style="color:var(--muted)">No women's wigs found for the selected criteria.</div>
        ...
        ?>
          <article class="card" data-price="<?php echo (float)($p['price'] ?? 0); ?>">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center">
              ...
              <?php if (!empty($p['sku'])): ?><div style="color:#6b7280">SKU: <?php echo h($p['sku']); ?></div><?php endif; ?>
            </div>
...
<aside id="cart-drawer" class="cart-drawer" aria-hidden="true" role="dialog" aria-label="Shopping cart">
  ...
  <div class="cart-footer">
    ...
    <div class="actions">
      ...
      <a href="index.php" id="continue-shopping" class="continue-btn">Continue</a>
    </div>
...
(function(){
  ...
  /* --------------------------
     Mobile category tap UX:
     - On narrow viewports (< 768 px), when a user taps a category link (.cat-item),
       show a full-screen overlay (spinner + message) while the browser navigates.
     - This provides the "open" effect in mobile as requested.
     -------------------------- */
  const mobileOverlay = document.getElementById('mobileCategoryOverlay');
  const mobileOverlayText = document.getElementById('mobileCategoryOverlayText');

  function isMobileViewport() {
    return window.innerWidth < 768;
  }

  // Delegate clicks on category links
  document.addEventListener('click', function(evt) {
    const a = evt.target.closest && evt.target.closest('.cat-item');
    if (!a) return;
    const href = a.getAttribute('href');
    if (!href) return;
    // On mobile viewports show overlay, then navigate
    if (isMobileViewport()) {
      evt.preventDefault();
      const label = (a.querySelector('span') && a.querySelector('span').textContent) ? a.querySelector('span').textContent.trim() : 'Category';
      if (mobileOverlay && mobileOverlayText) {
        mobileOverlayText.textContent = 'Opening ' + label + '...';
        mobileOverlay.classList.add('open');
        mobileOverlay.setAttribute('aria-hidden','false');
      }
      // small delay gives overlay a chance to render before navigation
      setTimeout(function(){
        window.location.href = href;
      }, 220);
    }
    // desktop behavior remains normal (no prevention)
  }, true);
